...
// DRO: Remove the local drag addAction for _unit — intended to be called via remoteExec on each client
// before re-adding drag actions after a unit respawns, preventing duplicate drag entries.
rev_removeDragAction = {
...
rev_addReviveToUnit = {
	...
	if (isMultiplayer) then {
		...
		_handlerRespawn = [_unit, ["Respawn", {
			...
			[_newUnit] remoteExec ["rev_removeDragAction", _allPlayers, false];

			// --- Re-add Event Handlers ---
			_newUnit addEventHandler ["HandleDamage", rev_handleDamage];
			_newUnit addEventHandler ["Killed", rev_handleKilled];
			_newUnit setCaptive false;
			reviveUnits = reviveUnits - [_oldUnit];
			reviveUnits pushBack _newUnit;
			publicVariable 'reviveUnits';
			[_newUnit] remoteExec ["rev_reviveActionAdd", _allPlayers, true];
			[_newUnit] remoteExec ["rev_dragActionAdd", _allPlayers, true];
