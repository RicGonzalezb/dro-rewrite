...

{
	...
	if (isMultiplayer) then {
		if (!isPlayer _x) then {
			_handlerLocal = [_x, ["Local", DRO_fnc_changeLocal]] remoteExec ["addEventHandler", 0, true];
		};		
		_handlerDamage = [_x, ["HandleDamage", DRO_fnc_handleDamage]] remoteExec ["addEventHandler", _x, true];	
		_handlerKilled = [_x, ["Killed", DRO_fnc_handleKilled]] remoteExec ["addEventHandler", _x, true];
		...
		_handlerDamage = _x addEventHandler ["HandleDamage", DRO_fnc_handleDamage];
		_handlerKilled = _x addEventHandler ["Killed", DRO_fnc_handleKilled];
		diag_log format ["Revive initialized for unit %1", _x];	
	};	
