...
**Quando usar:** Este guia é lido **uma única vez**, pelo **primeiro Master** designado para iniciar um projeto de refactor/desenvolvimento. Ele ensina como montar o projeto do zero: assessment, criação dos documentos de controle, estrutura de fases, e padrões de delegação.

**Depois de usado:** Uma vez que o primeiro Master criou o `_REFACTOR_PLAN.md` (com seção Cold Start) e o `_REFACTOR_PROGRESS.md`, esses dois documentos passam a ser a fonte de verdade. Masters subsequentes que assumam o projeto **não precisam ler este guia** — basta ler o Cold Start no PLAN e o PROGRESS para se situarem e continuarem o trabalho.

**Ciclo de vida do conhecimento:**

```
ESTE GUIA (leitura única)
    ↓ primeiro Master usa para...
_REFACTOR_PLAN.md (criado pelo primeiro Master)
    ├── Seção 0: Cold Start ← novo Master lê isto para se situar
    ├── Fases, regras, prompts dos workers
    └── Atualizado ao longo do projeto
_REFACTOR_PROGRESS.md (criado pelo primeiro Master)
    └── Log vivo de tudo que foi feito ← fonte de verdade do estado real
```

**Referência técnica (Seção 7):** A seção de boas práticas SQF é a única parte deste documento que pode ser consultada novamente — por workers ou masters — como referência técnica rápida. Opcionalmente, o Master pode copiar trechos relevantes para o briefing de workers.

---

## 1. Seu papel: Master Planner

Você é o **primeiro** prompt master designado para um projeto Arma 3 SQF. Seu objetivo é **montar a estrutura do projeto, planejar, coordenar e revisar** — não executar. O trabalho pesado (edições de código, correções de bugs, migrações) será delegado a outros prompts (worker prompts), que receberão instruções claras e escopo fechado.
...
Responsabilidade contínua (vale para todos os Masters, não só o primeiro):
...
O plano completo. A **primeira seção** deve ser sempre o **Cold Start** — um guia de onboarding para novos prompts master que assumem o projeto sem contexto prévio.

#### Seção 0: Cold Start

Projetos de refactor duram várias sessões. Cada sessão nova começa com contexto zerado. O Cold Start resolve isso — é a primeira coisa que um novo master lê para se situar em minutos. Deve conter:
...
1. **Seu papel:** Você é o prompt-mestre (gerente) de um projeto de [tipo] em [linguagem].
   Seu trabalho: entender o estado atual, planejar módulos, gerar prompts
   contextualizados para sessões worker executarem, e validar resultados.

2. **Leia `_REFACTOR_PROGRESS.md`** — log detalhado de tudo que foi feito.
   O status REAL do projeto está lá. Este PLAN.md contém a visão original,
   que pode estar desatualizada.

3. **Tabela rápida de status** (atualize ao completar módulos):
...
4. **Fluxo de trabalho:** Você NÃO executa os módulos diretamente — gera prompts
   autocontidos para sessões worker. O prompt deve conter todo o contexto necessário
   (caminho, regras, o que não tocar, formato de relatório).

5. **Idioma do usuário:** [idioma]

**Caminho do projeto:**
`[caminho completo]`

**Arquivos-chave:**
- `_REFACTOR_PLAN.md` — este arquivo (plano geral + prompts dos módulos)
- `_REFACTOR_PROGRESS.md` — log de progresso (fonte de verdade)
- [listar diretórios e arquivos críticos do projeto]

**Regras universais (todos os módulos):**
- [Regras de namespace, prefixos]
- [O que NÃO tocar (código estabilizado)]
- [Padrões obrigatórios (CBA, CfgFunctions, etc.)]
- [Formato de report dos workers]

**Procedimento de trabalho:**
- **Módulos planejados:** prompt fica NESTE arquivo, na seção do módulo.
  O usuário copia e cola no worker. Worker appenda relatório no PROGRESS.md.
- **Hotfixes avulsos:** master gera prompt no chat. Worker aplica e reporta no PROGRESS.md.
```

**Por que o Cold Start é essencial:**

- Contexto de LLM zera entre sessões — sem Cold Start, cada novo master perde tempo redescoberto o que já foi decidido
- Evita que um novo master tome decisões que contradizem o que já foi estabelecido
- A tabela de status dá visão instantânea do progresso sem precisar ler o progress inteiro
- As regras universais evitam que workers introduzam padrões inconsistentes
- O procedimento de trabalho define o fluxo master→worker→validação sem ambiguidade

**Manutenção do Cold Start:**

- Atualize a tabela de status sempre que um módulo for concluído
- Adicione novos arquivos-chave quando surgirem durante o projeto
- Se uma regra universal mudar (ex: nova dependência, novo padrão), atualize aqui
- O Cold Start deve ser suficiente para um master novo iniciar em <5 minutos de leitura

#### Seção restante: Phases + Rules
