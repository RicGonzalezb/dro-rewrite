...
while {true} do {
	...
	_playerGroupUnique = ((units _group + ([] call CBA_fnc_players)) arrayIntersect (units _group + ([] call CBA_fnc_players)));
	{
	...
	if (count _unitPositions > 0) then {
		...
		*/
		...
		_dir = [trgAOC, (getPos (selectRandom _playerGroupUnique))] call BIS_fnc_dirTo;
		_spawnPos = [(getPos (selectRandom _playerGroupUnique)), 200, _dir] call BIS_fnc_relPos;
		markerRespawnTeam setMarkerPos _spawnPos;
	};
