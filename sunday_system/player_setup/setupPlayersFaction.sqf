...
if (_resupplyValid) then {
	...
	if (!isNil "_resupplyPos") then {
		if (!([_resupplyPos, "resupply"] call DRO_fnc_validPos)) then {
			_resupplyValid = false;
		};
	} else {
		_resupplyValid = false;
...
switch (insertType) do {
				...
				switch (_chosenBoundary) do {
					...
					case 2: {
						...
						for "_i" from 0 to ([2, 4] call BIS_fnc_randomInt) do {
							...
							if ([_thisPos, "player-crate"] call DRO_fnc_validPos) then {
	...
	case 2: {
		...
		*/
		...
		[_playerGroupUnique] execVM 'sunday_system\player_setup\callParadrop.sqf';
	...
	case 3: {
		...
		// Draw drop marker
		deleteMarker "campMkr";
		_campName = format ["LZ %1", ([FOBNames] call sun_selectRemove)];
		missionNameSpace setVariable ["publicCampName", _campName];
		publicVariable "publicCampName";
		markerPlayerStart = createMarker ["campMkr", _randomStartingLocation];
		markerPlayerStart setMarkerShape "ICON";
...
//play music on insert
//[[musicMain, 0 , 1], "bis_fnc_playmusic", true] call BIS_fnc_MP;

//insertType = ["GROUND","SEA","HALO","HELI"];
if ((worldName in ["Cam_Lao_Nam","vn_khe_sanh","vn_the_bra"]) && (insertType == "HELI") && (timeOfDay <= 2)) then {
	[musicMainVNHeli, 0, 0.7] remoteExec ["BIS_fnc_playMusic", ([0, -2] select isDedicated)];
} else {
...
// Lights, backpacks, reset action, unit traits, force allowDamage to account for any setup issues
{
	...
	if (sunOrMoon < 0.9) then {
		_thisUnit enableGunLights missionAIWeaponLight;
	};
