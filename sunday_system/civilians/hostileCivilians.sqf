...
_A3weapons = ["hgun_Pistol_01_F","arifle_AKS_F","arifle_AKM_F","sgun_HunterShotgun_01_F","sgun_HunterShotgun_01_sawedoff_F"];
_DLCweapons = [];

//detect gear-specific addons
if ((configfile >> "CfgMods" >> "vn") call BIS_fnc_getCfgIsClass) then {
	_DLCweapons = ["vn_tt33","vn_pm","vn_ak_01","vn_sks","vn_pps43","vn_k50m","vn_izh54_shorty","vn_m38"];
};

if ((configfile >> "CfgMods" >> "SPE") call BIS_fnc_getCfgIsClass) then {
	_DLCweapons = ["SPE_Fusil_Mle_208_12","SPE_Fusil_Mle_208_12_Sawedoff","SPE_K98","SPE_MP40","SPE_Sten_Mk2","SPE_1911","SPE_P08"];
};

if ((configfile >> "CfgMods" >> "IF") call BIS_fnc_getCfgIsClass) then {
	_DLCweapons = ["LIB_K98","LIB_P38","LIB_M9130","LIB_TT33","LIB_LeeEnfield_No1","LIB_Colt_M1911","LIB_LeeEnfield_No4","LIB_Webley_mk6"];
};
...
_switchToHostile = {
	...
	[_unit, _DLCweapons, _goggles] spawn {
		params ["_unit", "_DLCweapons", "_goggles"];
					...
					if (_x != _unit) then {
						if (_x distance _unit < 70) then {
							...
							(units group _x) joinSilent _group;							
							{
								...
								if ((count _DLCweapons) > 0) then {
									removeAllWeapons _x;
									_sDLCWeapon = (selectRandom _DLCweapons);
									_mag = (getArray (configFile >> "CfgWeapons" >> _sDLCWeapon >> "magazines")) select 0;
									for "_i" from 2 to (random 4) do {_this addMagazines _mag};
									_this addWeapon _sDLCWeapon;
								};
