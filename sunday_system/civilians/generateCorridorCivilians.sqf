...
private _centerSide = createCenter sideLogic;

{
	...
	// Create spawn point modules (where civs CAN appear)
	{
		(createGroup _centerSide) createUnit ["ModuleCivilianPresenceUnit_F", _x, [], 0, "FORM"];
	} forEach _spawnPositions;

	// Create safe spots at buildings so civs have places to hang out
	{
		if (_forEachIndex >= 4) exitWith {};  // max 4 safe spots per location
		private _ssUnit = (createGroup _centerSide) createUnit ["ModuleCivilianPresenceSafeSpot_F", (getPos _x), [], 0, "FORM"];
		{
	...
	private _modCivs = (createGroup _centerSide) createUnit ["ModuleCivilianPresence_F", _locPos, [], 0, "FORM"];
	...
	diag_log format ["DRO: Corridor civs at %1: %2 units, %3 spawn points, %4 safe spots, area %5m",
		_locName, _unitCount, count _spawnPositions, (count _buildings) min 4, _areaSize];
