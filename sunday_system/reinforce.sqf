...
for "_i" from 1 to _numReinforcements do {	
	...
	if (count enemyGVPool > 0) then {
		{
			if (([_x] call DRO_fnc_getTrueCargo) >= 4) exitWith {			
	...
	if (!isNil "_reinforceType") then {
		switch (_reinforceType) do {
			case "INFANTRY": {
				...
				if (([_spawnPos, "reinforce"] call DRO_fnc_validPos) && {(({(_spawnPos distance _x) < 600} count (units (grpNetId call BIS_fnc_groupFromNetId))) == 0)}) then {
			...
			case "HELI": {
				...
				if (!([_spawnPos, "reinforce"] call DRO_fnc_validPos)) exitWith {};
