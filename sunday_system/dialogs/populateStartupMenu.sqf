...
if ((configfile >> "CfgMods" >> "SPE") call BIS_fnc_getCfgIsClass) then {
	_modList = composeText [_modList, lineBreak, "Spearhead 1944"];
};
if ((configfile >> "CfgMods" >> "vn") call BIS_fnc_getCfgIsClass) then {
	_modList = composeText [_modList, lineBreak, "S.O.G. Prairie Fire"];
};
if ((configfile >> "CfgMods" >> "ace") call BIS_fnc_getCfgIsClass) then {
...
_warningList = "";
if ((configfile >> "CfgMods" >> "SPE") call BIS_fnc_getCfgIsClass) then {
	if (SPE_revive_system_mode == 3) then {
		_warningList = composeText [_warningList, lineBreak, "Spearhead Medical disabled"];
	};
};
if ((configfile >> "CfgMods" >> "vn") call BIS_fnc_getCfgIsClass) then {
...
if ((configfile >> "CfgPatches" >> "ace_advanced_fatigue") call BIS_fnc_getCfgIsClass) then {
	if (!isNil "ace_advanced_fatigue_enabled") then {
		...
		if ((ace_advanced_fatigue_enabled) && ((["Stamina", 0] call BIS_fnc_getParamValue) == 1)) then {
			_warningList = composeText [_warningList, lineBreak, "ACE Advanced Fatigue is loaded, DRO stamina should be enabled."];
		};
	};
};
if ((configfile >> "CfgPatches" >> "C2_CORE") call BIS_fnc_getCfgIsClass) then {
