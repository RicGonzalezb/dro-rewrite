...
### Sea insert — spawn priorizando perpendicular à praia — 2026-07-03 (Master/Opus)
Sintoma (Gonza): spawn dos barcos às vezes margeando a costa (corredor paralelo) em vez de vir perpendicular ao desembarque. Causa: a busca de spawn (fn_findSeaCorridor, etapa 3) escolhia o ângulo com MAIOR alcance de água aberta entre `[-40,-20,0,20,40]` -> oblíquo ganhava em costa curva. Fix: score = `_clearTo - (abs _x) * _anglePenalty` (`_anglePenalty=8` m/grau, tunável); perpendicular (0deg) vence salvo se um oblíquo tiver muito mais água aberta. Set de ângulos ampliado p/ `[-40,-20,-10,0,10,20,40]` (resolução fina perto do perpendicular). Viabilidade preservada: `_bestSpawn` só é setado p/ lanes >=300m, então a disponibilidade não muda — só a preferência de ângulo. `git add -f functions/fn_findSeaCorridor.sqf _DRO_REFACTOR_PROGRESS.md`.

### Sea insert — baseline do spawn = normal real da praia (fix do oblíquo) — 2026-07-03 (Master/Opus)
Sintoma (Gonza, print Altis/Pefkas Bay): apesar do scoring, spawn saiu bem oblíquo cruzando a baía. Causa-raiz: o baseline "perpendicular" era `dirTo(origin -> drop)`. Quando o centro do AO está deslocado ao longo da costa (não em frente à praia), essa direção já nasce torta -> os offsets ±40 oscilam em torno de um baseline errado e nunca alcançam a perpendicular real. Fix (fn_findSeaCorridor, dentro do `if (_foundDrop)`): calcular a NORMAL LOCAL da praia — varredura de 24 direções (passo 15°, até 220m) achando a terra mais próxima = `_landward`; `_seaward = _landward + 180`. Esse `_seaward` (independente do centro) vira o baseline do scoring E a direção landward do refino do drop. Agora 0° = perpendicular verdadeiro à praia. `git add -f functions/fn_findSeaCorridor.sqf _DRO_REFACTOR_PROGRESS.md`.

### Sea insert — desembarque 1.5m + garantia de raso + no-collision — 2026-07-03 (Master/Opus)
Gonza: ejeção ainda em água funda; pediu 1.5m + desabilitar colisão unidade<->barco (atropelamento). Fix (fn_boatInsertion, PFH de ejeção):
- Thresholds: `_wadeable` (primário) `_depth > -1.5`; `_arrived` `> -2`; `_stuckNear` simplificado p/ `(_stall>=4) && _pastSpawn` (travou em qualquer profundidade).
- **Garantia de raso:** na hora de ejetar, se `_depth < -1.5` (barco travado/timeout ainda fundo), `setPosASL` do barco no seu drop raso ANTES de largar o pessoal -> ninguém desembarca em água funda. Nos casos normais (chega raso via arrived/wadeable) não há teleporte.
...
### Off-map audit — diag no DRO_fnc_validPos (rejeicao nao fica silenciosa) — 2026-07-04 (Master/Opus)
`fn_validPos.sqf` ganhou 2o param opcional `_tag`. Quando uma tag nao-vazia e passada, uma rejeicao loga no .rpt: `DRO: validPos REJECT [tag] pos=... -> spawn skipped`. Sem tag (ex: pools de posicao do generateAOLocation, onde falha e normal) fica quieto = sem spam.
Tags nos sites de spawn discreto: reinforce(4), reactive-hvt/veh(2), pow-hostage(1), bunker(1), insert-group(2), support-arty(1), reset-ai(4), friendly/friendly-rdv(3), resupply/player-insert/player-crate(6), disarm-ied(1). objGrouping ja tem diag proprio; generateAOLocation deixado sem tag.
Verificacao: escrita atomica; balanco delta 0; sem CR.
`git add -f functions/fn_validPos.sqf sunday_system/reinforce.sqf sunday_system/objectives/selectReactiveTask.sqf sunday_system/objectives/pow.sqf sunday_system/generate_enemies/generateBunker.sqf sunday_system/orders/insertGroup.sqf sunday_system/player_setup/addSupports.sqf functions/fn_resetAI.sqf sunday_system/player_setup/resetAIAction.sqf sunday_system/player_setup/generateFriendlies.sqf sunday_system/player_setup/setupPlayersFaction.sqf sunday_system/objectives_neutral/disarmIED.sqf _DRO_REFACTOR_PROGRESS.md`